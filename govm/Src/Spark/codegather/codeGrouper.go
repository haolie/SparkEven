...
func (this *CodeGrouper) Start(ctx context.Context) (errStr Model.Err) {
	...
	faces, errStr := gatherDateFace(ctx, conn, this.date)
	...
	for _, item := range faces {
		...
		if item.Code > 1680000 {
			continue
		}

		tempArray[index] = item.Code
		index++
		if index == this.count {
			index = 0
