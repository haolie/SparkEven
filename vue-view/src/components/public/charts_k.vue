...
<script>
  ...
  export default {
    ...
    methods:{
      setChart(){
        if(!this.dataItems||!this.$refs.kchart)return
        var dates=[]
        var data=[]
        var volumes=[]
        this.dataItems.forEach(function (d) {
          dates.push(d.date)
          data.push([d.startprice,d.lastprice,d.min,d.max])
        ...
        this.chart=this.$echarts.init(this.$refs.kchart);
          ...
          xAxis: [{
            ...
            axisLabel: {
              formatter: function (value) {
                return _this.$echarts.format.formatTime('MM-dd', value);
              }
            },
            min: 'dataMin',
            max: 'dataMax',
