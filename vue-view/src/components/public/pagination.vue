  ...
  export default{
    ...
    computed: {
      ...
      grouplist: function () { // 获取分页页码
        ...
        if (this.current > this.page - count) center = this.page - count
        ...
        if (this.page > this.pagegroup) {
          if (this.current > count + 1) list.unshift({text: '...', val: list[0].val - 1})
          if (this.current < this.page - count) list.push({text: '...', val: list[list.length - 1].val + 1})
        }
        return list
      }
    },
