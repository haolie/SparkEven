  ...
  if (results && results.length)
    for (var i in results) {
      items.push({
        ...
        volume:results[i].volume/100,
        turnoverRate:results[i].turnoverRate/100,
        turnover:results[i].turnover/100,
        ...
        async.mapLimit(lists, 1, function (list, mapcallback) {
          ...
          for (var i in list) {
            ...
            str += '(' +
              ...
              (module.exports.getValueSql(item, 'volume') * 100) + ',' +
              (module.exports.getValueSql(item, 'turnoverRate') * 100) + ',' +
              (module.exports.getValueSql(item, 'turnover') * 100) + ',' +
...
suporter.prototype.updatacodeface = function (item, callback) {
  ...
  module.exports.getConnction(function (err, conn) {
    var fun = function (option, cb) {
      ...
      var faids = ["state", "_min", "_max", "face", "dde", "dde_s", "dde_b", "ud|_change","lastprice", "per", "startprice","volume", "turnoverRate","turnover"];
      ...
      async.mapLimit(lists,1,function(list,aycb){
        ...
        for(var i in list){
          ...
          for(var j in faids){
            ...
            var value = item[sf];
            if (sf == "lastprice" || sf == "ud" ||sf== "_min"
              || sf== "_max" || sf == "startprice"||sf == "volume"
              || sf == "turnoverRate"||sf == "turnover") value =Math.floor( value * 100);
